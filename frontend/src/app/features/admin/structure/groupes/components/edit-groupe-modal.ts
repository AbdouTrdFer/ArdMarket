...
export interface EditableGroupe {
  code: string;
...
@Component({
  selector: 'app-edit-groupe-modal',
  standalone: true,
  imports: [Modal, Button, FormsModule, DropdownFilter],
  template: `
    <app-modal
      title="Éditer le groupe"
      description="Modifiez les informations du groupe sélectionné."
      [open]="open()"
      ...
      <form modal-body class="space-y-6">
        ...
        <fieldset class="space-y-3">
          <div class="flex items-center gap-2">
            ...
            <label for="edit-code" class="block text-sm font-bold text-principalText uppercase tracking-wide">Code du groupe</label>
          </div>
          <input
            id="edit-code"
            type="text"
        ...
        <fieldset class="space-y-3">
          <div class="flex items-center gap-2">
            ...
            <label for="edit-students" class="block text-sm font-bold text-principalText uppercase tracking-wide">Nombre d'étudiants</label>
          </div>
          <input
            id="edit-students"
            type="number"
            [value]="groupe()?.students ?? ''"
...
export class EditGroupeModal {
  readonly open = input(false);
  readonly groupe = input<EditableGroupe | null>(null);
